...
# Summary with Wald and LRT p-values
epiDisplay::logistic.display(mv_reg_actt, simplified = FALSE, crude = FALSE)

# Visual check of model assumptions
performance::check_model(mv_reg_actt)

# Indices of model performance
performance::model_performance(mv_reg_actt)

# Check for multicollinearity
performance::check_collinearity(mv_reg_actt, ci = NULL)
```
...
# Summary with Wald and LRT p-values
epiDisplay::logistic.display(step_mv_reg_actt, simplified = FALSE, crude = FALSE)

# Visual check of model assumptions
performance::check_model(step_mv_reg_actt)

# Indices of model performance
performance::model_performance(step_mv_reg_actt)

# Check for multicollinearity
performance::check_collinearity(step_mv_reg_actt, ci = NULL)
```
...
```{r}
# Compare performance of different models
performance::compare_performance(mv_reg_actt, step_mv_reg_actt, verbose = TRUE)
# Radar plot to compare models
plot(performance::compare_performance(mv_reg_actt, step_mv_reg_actt, rank = TRUE, verbose = TRUE))

# Likelihood Ratio Test (LRT for comparison)
performance::test_likelihoodratio(step_mv_reg_actt, mv_reg_actt)
```
